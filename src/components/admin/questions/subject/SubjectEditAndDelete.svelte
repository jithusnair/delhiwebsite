<script>
    ...
    $: if(editMode) {
        let copy = {...subject};
        subjectCopy = String(copy.subjectTitle);
    }
    ...
    function update() {
        ...
        fetchOptions = {
            ...
            credentials: 'include', 
            body: JSON.stringify({_id: subject._id, subjectTitle: subjectCopy}),
        };
    ...
    {:else}
        <div class="input">
            <input 
                value = {subjectCopy}
                type="text"
                on:input = {(event)=> subjectCopy = event.target.value}
                placeholder="Type in the subject name">
